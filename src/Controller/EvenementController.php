...
class EvenementController extends AbstractController
{
    ...
    public function new(Request $request, EvenementRepository $evenementRepository, MailerInterface $mailer,UtilisateurRepository $utilisateurRepository): Response
    {    
        $data1 = $this->get('session')->get('data', []);
        $utilisateur = $utilisateurRepository->find($data1['id'] ?? '');
    ...
    public function exportEventAction(Evenement $evenement)
 {
    ...
    public function TriD(EvenementRepository $repository,Request $request,PaginatorInterface $paginator)
    {
        ...
        $evenement = $paginator->paginate(
            ...
            $request->query->getInt('page', 1),
                4);
        return $this->render("front/GestionEvent/evenement.html.twig", array(
